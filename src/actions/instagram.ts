...
const tokenExpiresAt = import.meta.env.FACEBOOK_ACCESS_TOKEN_EXPIRES_AT;

const CACHE_DURATION = 60 * 1000;
const INSTAGRAM_ID = "17841413948950087";

export const getInstagramFollowers = defineAction({
  handler: async () => {
    // 캐시된 데이터가 유효한지 확인
    const now = Date.now();
    if (
    ...
    if (!tokenExpiresAt) {
      throw new Error(
        "FACEBOOK_ACCESS_TOKEN_EXPIRES_AT이 설정되지 않았습니다.",
      );
    }
