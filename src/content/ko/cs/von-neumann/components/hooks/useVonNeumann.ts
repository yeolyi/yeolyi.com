...
export function useVonNeumann(lang: string) {
  const t = useTranslations(lang)("VonNeumann");
  const [code, setCode] = useState<string>(t.Simulator.defaultCode);
  ...
  const handleLoad = () => {
    ...
    const parsedCode = lines.map((line) => {
      ...
      if (info.type === "R-Type") {
        const rd =
          REGISTERS[args[0].toUpperCase() as keyof typeof REGISTERS] || 0;
        const rs =
          REGISTERS[args[1].toUpperCase() as keyof typeof REGISTERS] || 0;
